...
import 'package:amap_flutter_map/src/types/marker_animated.dart';
...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  /// 搜索路径规划
  Future<bool?> mapDrivingRouteSearch(
    RouteSearch routeSearch, {
    required int mapId,
  }) {
    return channel(mapId).invokeMethod<bool>(
      'map#drivingRouteSearch',
      routeSearch.toMap(),
    );
  }

  /// 更新Marker的数据（平滑移动）
  Future<void> setTrackData(
    MarkerAnimated markerAnimated, {
    required int mapId,
  }) {
    return channel(mapId).invokeMethod<void>(
      'markers#setTrackData',
      markerAnimated.toMap(),
    );
  }

  /// 更新Marker的数据（平滑移动）
  Future<void> markerToAnimated(
      MarkerAnimated markerAnimated, {
        required int mapId,
      }) {
    return channel(mapId).invokeMethod<void>(
  ...
  /// 更新Marker的数据（停止平滑移动）
  Future<void> markerToStopAnimated(
      MarkerAnimated markerAnimated, {
        required int mapId,
      }) {
    return channel(mapId).invokeMethod<void>(
      'markers#stopAnimated',
      markerAnimated.toMap(),
    );
  }
  ...
  ///路径规划 返回路径对象
  Stream<MapDrivingRouteResultEvent> onDrivingRouteResult({required int mapId}) {
    return _events(mapId).whereType<MapDrivingRouteResultEvent>();
  }

  Stream<MarkerSmoothMoveEvent> onMarkerSmoothMove({required int mapId}) {
    return _events(mapId).whereType<MarkerSmoothMoveEvent>();
  }
  Future<dynamic> _handleMethodCall(MethodCall call, int mapId) async {
    switch (call.method) {
      ...
      case 'map#onTap':
        ...
        break;
      case 'cluster#onTap':
        _mapEventStreamController.add(ClusterTapEvent(mapId, call.arguments['items']));
        break;
      ...
      case 'map#onPoiTouched':
        ...
        break;
      case 'map#onDrivingRouteResult':
        try {
          _mapEventStreamController.add(
              MapDrivingRouteResultEvent(mapId, call.arguments['mapRoute']!));
        } catch (e) {
          print('map#onDrivingRouteResult error===>' + e.toString());
        }
        break;
      case 'marker#smoothMoveProgress':
        _mapEventStreamController.add(MarkerSmoothMoveEvent(
            mapId,
