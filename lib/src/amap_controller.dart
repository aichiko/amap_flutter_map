...
class AMapController {
  ...
  void _connectStreams(int mapId) {
    ...
    if (_mapState.widget.onDrivingRouteResult != null) {
      _methodChannel.onDrivingRouteResult(mapId: mapId).listen(
          (MapDrivingRouteResultEvent e) =>
              _mapState.widget.onDrivingRouteResult!(e.value));
    }
    ...
    if (_mapState.widget.onClusterTap != null) {
      _methodChannel.onClusterTap(mapId: mapId).listen(
          (ClusterTapEvent e) => _mapState.widget.onClusterTap!(e.value));
    }
    ...
    if (_mapState.widget.onSmoothMoveResult != null) {
      _methodChannel.onMarkerSmoothMove(mapId: mapId).listen(
          (MarkerSmoothMoveEvent e) =>
              _mapState.widget.onSmoothMoveResult!(e.value));
    }
    _methodChannel
  ...
  Future<void> setTrackData(Marker marker, List<LatLng> points,
      int totalDurationTime, int playSpeed, bool isPlaying) {
    return _methodChannel.setTrackData(
        MarkerAnimated.from(marker,
            points: points,
  ...
  Future<void> animatedMarkers(Marker marker, List<LatLng> points,
      {double duration = 0.25}) {
    return _methodChannel.markerToAnimated(
        MarkerAnimated.from(marker, points: points, duration: duration),
        mapId: mapId);
  }

  Future<void> animatedStopMarkers(Marker marker) {
    return _methodChannel.markerToStopAnimated(
        MarkerAnimated.from(marker, points: []),
        mapId: mapId);
  }

  Future<bool?> drivingRouteSearch(LatLng startLatLng, LatLng destinationLatLng,
      {strategy = 10, bool showTraffic = true}) {
    return _methodChannel.mapDrivingRouteSearch(
        RouteSearch.from(startLatLng, destinationLatLng,
            strategy: strategy, showTraffic: showTraffic),
        mapId: mapId);
  }
