...
QuadTreeNodeData QuadTreeNodeDataForAnnotation(MAPointAnnotation* poi)
{
...
{
    CLLocationDegrees minX = ((MAPointAnnotation *)pois[0]).coordinate.latitude;
    CLLocationDegrees maxX = ((MAPointAnnotation *)pois[0]).coordinate.latitude;
    
    CLLocationDegrees minY = ((MAPointAnnotation *)pois[0]).coordinate.longitude;
    CLLocationDegrees maxY = ((MAPointAnnotation *)pois[0]).coordinate.longitude;
    
    for (NSInteger i = 0; i < [pois count]; i++)
    {
        dataArray[i] = QuadTreeNodeDataForAnnotation(pois[i]);
...
- (NSArray *)getAnnotationsWithoutClusteredInMapRect:(MAMapRect)rect
{
    ...
    QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(rect), ^(QuadTreeNodeData data) {
        MAPointAnnotation *aPoi = (__bridge MAPointAnnotation *)data.data;
        
        CLLocationCoordinate2D coordinate = CLLocationCoordinate2DMake(aPoi.coordinate.latitude, aPoi.coordinate.longitude);
...
- (NSArray *)clusteredAnnotationsWithinMapRect:(MAMapRect)rect withZoomScale:(double)zoomScale andZoomLevel:(double)zoomLevel {
    ...
    for (NSInteger x = minX; x <= maxX; x++)
    {
        for (NSInteger y = minY; y <= maxY; y++)
        {
            ...
            QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(mapRect), ^(QuadTreeNodeData data)
                                      {
                                          ...
                                          [pois addObject:(__bridge MAPointAnnotation *)data.data];
                                      });
            ...
            {
                // 只有一个则显示为车辆图标
                MAPointAnnotation *annotation = (MAPointAnnotation *)[pois firstObject];
...
- (NSArray<ClusterAnnotation *> *)clusteredAnnotationsWithinMapRect:(MAMapRect)rect withDistance:(double)distance {
    __block NSMutableArray<MAPointAnnotation *> *allAnnotations = [[NSMutableArray alloc] init];
    QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(rect), ^(QuadTreeNodeData data) {
        [allAnnotations addObject:(__bridge MAPointAnnotation *)data.data];
    });
    
    NSMutableArray<ClusterAnnotation *> *clusteredAnnotations = [[NSMutableArray alloc] init];
    for (MAPointAnnotation *aAnnotation in allAnnotations) {
...
- (ClusterAnnotation *)getClusterForAnnotation:(MAPointAnnotation *)annotation inClusteredAnnotations:(NSArray<ClusterAnnotation *> *)clusteredAnnotations withDistance:(double)distance {
