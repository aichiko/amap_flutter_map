...
- (instancetype)init:(FlutterMethodChannel*)methodChannel
    ...
    if (self) {
        ...
        [_methodChannel addMethodName:@"markers#animated" withHandler:^(FlutterMethodCall * _Nonnull call, FlutterResult  _Nonnull result) {
            id animatedDic = call.arguments[@"markersToAnimated"];
            AMapMarker *markerModel = _markerDict[animatedDic[@"marker"]];
            if (!markerModel.animated) {
                result(nil);
                return;
            }
            
            NSArray<NSArray<NSNumber *> *> *latLng_arr = animatedDic[@"points"];
            int count = latLng_arr.count;
            
            CLLocationCoordinate2D s_coords[count];
            
            for (int i = 0; i < latLng_arr.count; i++) {
                NSArray<NSNumber *> *arr = latLng_arr[i];
                CLLocationCoordinate2D coor = CLLocationCoordinate2DMake(arr.firstObject.doubleValue, arr.lastObject.doubleValue);
                s_coords[i] = coor;
            }
            
            if ([markerModel.annotation isKindOfClass:[MAAnimatedAnnotation class]]) {
                
                MAAnimatedAnnotation *animatedAnnotation = (MAAnimatedAnnotation *)markerModel.annotation;
                
                CLLocationCoordinate2D coor = CLLocationCoordinate2DMake(s_coords[0].latitude, s_coords[0].longitude);
                [animatedAnnotation setCoordinate:coor];
                
                [animatedAnnotation addMoveAnimationWithKeyCoordinates:s_coords count:count withDuration:5 withName:nil completeCallback:nil];
            }
